...
class CollegeSearchTool:
    ...
    async def search(self, query_params: Dict, send_status_callback: callable) -> List[College]:
        """Search colleges using web search and AI processing"""
        try:
            # Build search query
            await send_status_callback("正在构建搜索查询...")
            search_query = self._build_search_query(query_params)
            
            # Search the web for university information
            await send_status_callback("正在实时搜索大学数据库与网站...")
            ...
            if search_results: # Only send this if there are results to process
                await send_status_callback("正在使用AI处理搜索结果，提取结构化大学信息...")
            ...
            await send_status_callback(f"搜索过程中发生错误: {e}") # Notify user of error
            return []
...
class UniGuideAgent:
            ...
            if isinstance(extracted_info["budget"], dict):
                ...
                try:
                    ...
                    if isinstance(parsed_budget, dict):
                     ...
                     # Optionally notify user if budget string is unusable for filtering
                     # await send_status_callback("您提供的预算信息格式未能完全解析，将尝试按文本描述处理。") # This would require send_status_callback here
    
    async def process_college_match(self, message: str, send_status_callback: callable) -> str:
        """Process college recommendation request"""
        await send_status_callback("正在分析您的学术背景与偏好...")
        ...
        colleges = await self.college_search.search(search_params, send_status_callback)
        
        if not colleges and not search_params: # If search was broad and still no results
             await send_status_callback("初步搜索未发现特定结果，尝试进行更广泛的咨询...")
        elif not colleges:
            await send_status_callback("根据您的详细要求，目前未能找到匹配的大学。尝试放宽一些条件或进行常规咨询。")

        await send_status_callback("正在计算与您匹配的大学得分...")
        ...
        await send_status_callback("正在生成个性化推荐报告...")
    ...
    async def process_message(self, message: str, send_status_callback: callable) -> str:
        ...
        if intent == IntentType.COLLEGE_MATCH:
            response = await self.process_college_match(message, send_status_callback)
        elif intent == IntentType.ESSAY_REVISE:
            response = "📝 **论文修改助手即将上线！** 目前，我建议您专注于展现您的个人故事和独特经历。需要大学推荐吗？"
        elif intent == IntentType.SCHEDULE_PLAN:
            response = "📅 **规划日程助手即将上线！** 我可以帮您了解申请截止日期。多数早申截止日期在11月1日，常规申请通常在1月1日。"
        else:
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    async def _send_status_update(status_message: str): # Define the callback function
        try:
        ...
        except Exception as e: # Catch potential errors during send (e.g., connection closed)
    ...
    try:
        while True:
            ...
            if user_message.strip():
                ...
                response = await agent_instance.process_message(user_message, _send_status_update)
